...
export function AddButton({ product }: { product: Product }) {
  ...
  return <><p className="mt-6 text-xl font-bold">{money(catalogUnitPrice(product,size,quantity))} each{quantity >= 12 && apparel ? " · team rate where eligible" : ""}</p><div className={`mt-8 grid gap-3 ${apparel ? "grid-cols-[140px_110px_1fr]" : "grid-cols-[110px_1fr]"}`}>
